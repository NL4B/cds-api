    ...
    return new Promise((resolve, reject) => {
        ...
        switch (cfDestinationInfo.Authentication) {
            ...
            case "OAuth2JWTBearer":
                if (cfDestinationInfo["jwts.type"] !== undefined) {
                    switch (cfDestinationInfo["jwts.type"]) {
                        case 'gcp_service_account':
                            ...
                            break;
                        default:
                            error = new Error(`CDS-API: Authentication Type OAuth2JWTBearer is only supported for gcp_service_account!`);
                            if (process.env.DEBUG === "true") {
                                console.error('Error in getAuthorizationHeader (destination.js) \n');
                                console.error(error.message);
                            }
                ...
                break;
            default:
                error = new Error(`CDS-API: Authentication ${cfDestinationInfo.Authentication} is not supported!`)
                if (process.env.DEBUG === "true") {
                    console.error('Error in getAuthorizationHeader (destination.js) \n');
                    console.error(error.message);
                }
...
function createJwtTokenForGCPServiceAccount(cfDestinationInfo) {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            error = new Error(`CDS-API: Client Secret isn't a Google Cloud Platfrom Service Account json file`)
            if (process.env.DEBUG === "true") {
        ...
        if (cfDestinationInfo['jwts.expiration-time']) {
            ...
            if (!Number.isNaN(et) && Number.isInteger(et)) {
                if (et < 0) {
                    error = new Error(`CDS-API: property jwts.expiration-time cannot be negative`)
                    if (process.env.DEBUG === "true") {
                        console.error('Error in createJwtTokenForGCPServiceAccount (destination.js) \n');
                        console.error(error.message);
                    }
                ...
                error = new Error(`CDS-API: property jwts.expiration-time is not a integer`)
                if (process.env.DEBUG === "true") {
                    console.error('Error in createJwtTokenForGCPServiceAccount (destination.js) \n');
                    console.error(error.message);
                }
...
class destinations {
    constructor(destinationConfiguration) {
        if (destinationConfiguration.URL === undefined) {
            throw new Error(`CDS-API: Url is not defined!`);
        }
        if (destinationConfiguration.Authentication !== "NoAuthentication"
            ...
            throw new Error(`CDS-API: Authentication Type ${destinationConfiguration.Authentication} is not supported!`);
        }
                ...
                case "Internet":
                    switch (this.destinationConfiguration.Type) {
                        case "HTTP":
                            ...
                            break;
                        default:
                            reject(new Error(`CDS-API: Type ${this.destinationConfiguration.Type} is not supported!`));
                    }
                    break;
                default:
                    reject(new Error(`CDS-API: Proxy Type ${this.destinationConfiguration.ProxyType} is not supported!`));
            }
        });
    }
}
